...
namespace MercadoEnvio.Historial_Cliente
{
    public partial class HistorialComprasCalificadas : Form
    {
        ...
        private void cargar_grid_histComprasCalificadas()
        {
            dataGridViewComprasCalificadas.Rows.Clear();
            ...
            while (lectura.Read())
            {
               ...
               filas[filas.Count - 1].CreateCells(dataGridViewComprasCalificadas, columnas);
                
            }

            lectura.Close();
            dataGridViewComprasCalificadas.Rows.AddRange(filas.ToArray());
        }
